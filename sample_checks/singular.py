...
def do_nu_nubar_plot(xvals, isnu, weights, bins, logx, logy, xlabel, ylabel,
    ...
    if len(isnu) != 0:
        ...
        pyplot.savefig("TestPlots/%s/%s.pdf"%(savedir,savename))
...
def do_total_plot(all_data, all_weights, bins, logx, xlabel,
    ...
    pyplot.savefig("TestPlots/%s/%s_Components_Log.pdf"%(savedir,savename))

    pyplot.yscale("linear")
    pyplot.ylim(0.0,1.1*ymax)
    pyplot.savefig("TestPlots/%s/%s_Components.pdf"%(savedir,savename))
    ...
    pyplot.ylim(numpy.power(10,int(numpy.log10(ymin))-1),
                numpy.power(10,int(numpy.log10(ymax))+1))
    pyplot.savefig("TestPlots/%s/%s_Stack_Log.pdf"%(savedir,savename))

    pyplot.yscale("linear")
    pyplot.ylim(0.0,1.1*ymax)
    pyplot.savefig("TestPlots/%s/%s_Stack.pdf"%(savedir,savename))
    ...
    pyplot.ylim(numpy.power(10,int(numpy.log10(ymin))-1),
                numpy.power(10,int(numpy.log10(ymax))+1))
    pyplot.savefig("TestPlots/%s/%s_Log.pdf"%(savedir,savename))
...
def do_resolution_plot(truth_data, reco_data, truth_energy, weights, evals,
    ...
    for i in range(0,len(evals)-1):
        ...
        if numpy.sum(histx) != 0.0:
            ...
            pyplot.subplots_adjust(bottom=0.12,top=0.8)
            pyplot.title(title,size='x-large',x=0.5,y=1.20)
            pyplot.savefig("TestPlots/%s/%s_InBin_%.2f_%.2f.pdf"%(savedir,savename,evals[i],evals[i+1]))
            pyplot.close()
    ...
    pyplot.subplots_adjust(bottom=0.12,top=0.8)
    pyplot.title(title,size='x-large',x=0.5,y=1.20)
    pyplot.savefig("TestPlots/%s/%s_Median.pdf"%(savedir,savename))
    pyplot.close()
